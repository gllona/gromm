...
/**
 * Class for ORM proxy of type MULTIPLE_DIRECT
 * 
 ...
 */
class ORMpairmapMultipleDirect extends ORMpairmap
{
    ...
    private function fetch_id_or_ids ()
    {
        ...
        $tablename  = ORMmd::table_name( ORMmd::get_fieldspecs_classname( $this->fieldspecs_owntable_forward ) );
        $fieldname  = ORMmd::get_fieldspecs_backlink( $this->fieldspecs_owntable_forward );
    ...
    protected function instance_save_delegate ()
    {
        ...
        $tablename = ORMmd::table_name( ORMmd::get_fieldspecs_classname( $this->fieldspecs_owntable_forward ) );
        $fieldname = ORMmd::get_fieldspecs_backlink( $this->fieldspecs_owntable_forward );
        // iterate over the ormObjects where the relation values should be saved
        foreach ( array_keys( $this->references ) as $target_id ) {
            $res = self::$ci->db->where( "id = $target_id" )
                                ->update( $tablename, array( $fieldname, $this->orm_id ) );
            if ( $res === null ) {
                throw new ORMexception( "SQL query error :: [ " . self::$ci->db->last_query() . " ]" );
            }
        }
